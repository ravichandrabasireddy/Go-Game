...
class GO:
    ...
    def detect_neighbor_moves(self, board, i, j):
        '''
        Detect all the neighbors of a given stone on a given board.
    ...
    def detect_neighbor_ally_moves(self, board, i, j):
        '''
        Detect the neighbor allies of a given stone on given board.
    ...
    def ally_dfs_moves(self, board, i, j):
        '''
        Using DFS to search for all allies of a given stone on a given board.
    ...
    def empty_cells(self,player):
        ...
        for i in range(self.size):
            for j in range(self.size):
                if self.valid_place_check(i, j, player, test_check = True):
                    possible_placements.append((i,j))
        random.shuffle(possible_placements)
        return possible_placements

    def empty_cells_moves(self,board,player):
        '''
        check valid moves for a player for a given board
        ...
        for i in range(self.size):
            for j in range(self.size):
                if self.valid_place_check_moves(board, i, j, player, test_check = True):
                    possible_placements.append((i,j))
        random.shuffle(possible_placements)
        return possible_placements
    ...
    def find_liberty_moves(self, board, i, j):
        '''
        Find liberty of a given stone on a given board. If a group of allied stones has no liberty, they all die.
        ...
        :return: boolean indicating whether the given stone still has liberty.
        '''
        ally_members = self.ally_dfs_moves(board, i, j)
        for member in ally_members:
            neighbors = self.detect_neighbor_moves(board, member[0], member[1])
            for piece in neighbors:
                # If there is empty space around a piece, it has liberty
                if board[piece[0]][piece[1]] == 0:
                    return True
